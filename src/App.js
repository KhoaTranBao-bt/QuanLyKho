...
const ZONES_COLLECTION = 'inventory_zones'; // Collection mới cho Vùng
...
export default function App() {
  ...
  const [zones, setZones] = useState([]); // Danh sách vùng
  const [activeZone, setActiveZone] = useState('ALL'); // Vùng đang chọn: 'ALL', 'UNCATEGORIZED', hoặc ID vùng
  ...
  useEffect(() => {
    ...
    const unsubItems = onSnapshot(qItems, (snapshot) => {
      ...
      // Update realtime cho trang chi tiết
      if (selectedItem) {
  ...
  const handleAddZone = async () => {
    const zoneName = window.prompt("Nhập tên khu vực mới (Ví dụ: Tủ A, Kệ 1):");
  ...
  const handleDeleteZone = async (zoneId, e) => {
    e.stopPropagation(); // Ngăn việc click xóa kích hoạt chọn vùng
    if (window.confirm("CẢNH BÁO: Xóa vùng này sẽ không xóa sản phẩm, nhưng sản phẩm sẽ chuyển về 'Chưa phân vùng'. Bạn chắc chắn chứ?")) {
      try { await deleteDoc(doc(db, ZONES_COLLECTION, zoneId)); }
      catch (e) { setError("Lỗi khi xóa vùng."); }
    }
  };

  // --- LOGIC XỬ LÝ ẢNH (ZOOM, PAN, PASTE) ---
  // ... (Giữ nguyên logic cũ) ...
  ...
  const onFileChange = (e) => {
    if (e.target.files && e.target.files.length > 0) {
      ...
      reader.addEventListener('load', () => { setImageSrc(reader.result); setIsCropping(true); setError(''); setCrop({ unit: '%', width: 100, height: 100, x: 0, y: 0 }); });
  ...
  const onLoad = (img) => { imgRef.current = img; setCrop({ unit: '%', width: 100, height: 100, x: 0, y: 0 }); };
  const showCroppedImage = async () => {
    if (!completedCrop || !imgRef.current) { setNewItemImage(imageSrc); setIsCropping(false); setImageSrc(null); return; }
    try { const base64 = await getCroppedImg(imgRef.current, completedCrop, 'newFile.jpeg'); setNewItemImage(base64); setIsCropping(false); setImageSrc(null); } catch (e) { setError("Lỗi cắt ảnh."); }
  };
  ...
  const handleAddItem = async (e) => {
    ...
    try {
      ...
      await addDoc(collection(db, ITEMS_COLLECTION), {
        ...
        image: finalImageUrl,
        description: "",
        zoneId: activeZone === 'ALL' || activeZone === 'UNCATEGORIZED' ? null : activeZone, // Lưu ID vùng
        createdAt: serverTimestamp(),
        createdBy: user.uid
      });
  ...
  const filteredItems = items.filter(item => {
    ...
    if (activeZone === 'ALL') {
      matchesZone = true; // Hiện tất cả
    } else if (activeZone === 'UNCATEGORIZED') {
      matchesZone = !item.zoneId || item.zoneId === null; // Hiện cái chưa có vùng
    } else {
      matchesZone = item.zoneId === activeZone; // Hiện theo ID vùng
    }
    ...
    <div className="min-h-screen bg-slate-50 text-slate-900 font-sans pb-10">
      ...
      <header className="bg-blue-600 text-white shadow-lg sticky top-0 z-20 px-4 pt-4 pb-2">
        <div className="flex justify-between items-center mb-4">
          ...
          {/* Chỉ hiện nút Thêm Mới khi đang ở trong một Vùng cụ thể (không phải ALL hay UNCATEGORIZED) */}
        ...
        {/* --- THANH MENU VÙNG (ZONES) --- */}
        <div className="flex gap-2 overflow-x-auto pb-2 no-scrollbar">
          {/* Tab Tất Cả */}
          <button 
            onClick={() => { setActiveZone('ALL'); setIsFormOpen(false); }}
            className={`whitespace-nowrap px-4 py-2 rounded-lg font-bold transition flex items-center gap-2 ${activeZone === 'ALL' ? 'bg-white text-blue-600 shadow-md' : 'bg-blue-700 text-blue-100 hover:bg-blue-500'}`}
          >
            <LayoutGrid size={18}/> Tất cả
          </button>

          {/* Tab Chưa Phân Vùng (Dữ liệu cũ) */}
          <button 
            onClick={() => { setActiveZone('UNCATEGORIZED'); setIsFormOpen(false); }}
            className={`whitespace-nowrap px-4 py-2 rounded-lg font-bold transition flex items-center gap-2 ${activeZone === 'UNCATEGORIZED' ? 'bg-white text-blue-600 shadow-md' : 'bg-blue-700 text-blue-100 hover:bg-blue-500'}`}
          >
            <AlertCircle size={18}/> Chưa phân vùng
          </button>
          ...
          {zones.map(zone => (
            <div key={zone.id} className="relative group">
              <button 
                ...
                className={`whitespace-nowrap px-4 py-2 rounded-lg font-bold transition flex items-center gap-2 pr-8 ${activeZone === zone.id ? 'bg-white text-blue-600 shadow-md' : 'bg-blue-700 text-blue-100 hover:bg-blue-500'}`}
              >
              ...
              <button 
                ...
                className="absolute right-1 top-1/2 -translate-y-1/2 p-1 text-blue-300 hover:text-red-300 opacity-50 hover:opacity-100 transition"
                title="Xóa vùng này"
              >
                <X size={14}/>
              </button>
          ...
          <button 
            ...
            className="whitespace-nowrap px-4 py-2 rounded-lg font-bold border-2 border-blue-400 border-dashed text-blue-100 hover:bg-blue-500 hover:text-white transition flex items-center gap-2"
          >
            <Plus size={18}/> Tạo vùng
          </button>
        </div>
      </header>
      ...
      <main className="max-w-5xl mx-auto px-4 py-8">
        ...
        {/* --- FORM THÊM MỚI (CHỈ HIỆN KHI CHỌN VÙNG) --- */}
            ...
            {isCropping ? (
              <div className="flex flex-col gap-4 animate-in fade-in">
                <div className="relative h-96 w-full bg-slate-900 rounded-xl overflow-hidden border-4 border-blue-500 shadow-2xl flex justify-center items-center p-4">
                  <ReactCrop crop={crop} onChange={(c) => setCrop(c)} onComplete={(c) => setCompletedCrop(c)} aspect={undefined}>
                    <img src={imageSrc} alt="Upload" onLoad={onLoad} style={{ maxHeight: '100%', maxWidth: '100%', objectFit: 'contain' }} />
                  </ReactCrop>
                </div>
              ...
              <form onSubmit={handleAddItem} className="space-y-6">
                  ...
                  <div className="w-2/3">
                    ...
                    {newItemImage ? (
                      ...
                      <label className="cursor-pointer bg-slate-50 hover:bg-slate-100 border-2 border-dashed border-slate-300 rounded-xl flex flex-col items-center justify-center h-64 text-slate-500 transition hover:border-blue-400 hover:text-blue-500">
                        ...
                        <span className="text-sm font-bold">Bấm chọn hoặc dán ảnh (Ctrl+V)</span>
                ...
                <button type="submit" disabled={isUploading} className="w-full bg-blue-600 text-white font-bold py-4 rounded-xl hover:bg-blue-700 shadow-lg transition flex justify-center items-center gap-2 disabled:bg-slate-400 text-lg">
                  <Save size={24} /> {isUploading ? "Đang lưu..." : "Lưu vào kho"}
                </button>
              </form>
            )}
          </div>
        ...
        <div className="relative mb-8">
          <input type="text" placeholder={`Tìm kiếm trong ${activeZone === 'ALL' ? 'tất cả kho' : 'vùng này'}...`} value={searchTerm} onChange={(e) => setSearchTerm(e.target.value)} className="w-full pl-12 pr-6 py-4 rounded-full border border-slate-200 shadow-md focus:ring-4 focus:ring-blue-100 focus:border-blue-500 outline-none text-lg" />
        ...
        {!loading && filteredItems.length === 0 ? (
          <div className="text-center py-16 text-slate-400 bg-white rounded-3xl border-2 border-dashed border-slate-200">
            ...
            <p className="text-xl font-medium">
              {activeZone === 'ALL' ? 'Kho chưa có gì cả' : 'Khu vực này đang trống'}
            </p>
            {activeZone !== 'ALL' && activeZone !== 'UNCATEGORIZED' && (
              <p className="text-sm mt-2 text-blue-500 cursor-pointer hover:underline" onClick={() => setIsFormOpen(true)}>Thêm món đầu tiên ngay</p>
            )}
          </div>
        ) : (
              ...
              <div key={item.id} className="bg-white rounded-2xl shadow-sm border border-slate-100 overflow-hidden flex flex-col hover:shadow-xl transition-shadow duration-300">
                ...
                <div className="p-5 flex-1 flex flex-col justify-between">
                  <div className="mb-4">
                    ...
                    {/* Hiển thị tên vùng nếu đang xem Tất cả */}
                    {activeZone === 'ALL' && (
                      <span className="text-xs font-bold px-2 py-1 bg-slate-100 text-slate-500 rounded-full">
                        {zones.find(z => z.id === item.zoneId)?.name || 'Chưa phân vùng'}
                      </span>
                    )}
                  </div>
                  <div className="flex items-center justify-between bg-slate-50 p-3 rounded-xl border border-slate-100 min-h-[60px]">
                    {editingId === item.id ? (
                      ...
                      <div className="flex items-center justify-between w-full">
                         ...
                         <button onClick={() => startEditingQty(item)} className="bg-white border border-slate-200 text-slate-600 px-4 py-2 rounded-lg font-bold text-sm shadow-sm hover:bg-blue-50 hover:text-blue-600 hover:border-blue-200 transition flex items-center gap-2"><Edit3 size={16}/> Sửa</button>
                      </div>
                    )}
                  </div>
                </div>
